...
export default function News({user,imagesPaths,page,news,disabled,initialNews,shelter}){
        ...
        if(navigator?.share || window?.AndroidHandler?.share){
            ...
            try{
                ...
                if(item?.image && item.image.length > 0){
                    ...
                    shareData.image = imagesPaths?.news+item.image;
                }
                /*
                ...
                */
                if(window?.AndroidHandler?.share){
                	...
                	if(shareData.image && shareData.image.length > 0){
                        ...
                        const imageUrlWithCacheBuster = shareData.image + (shareData.image.includes('?') ? '&' : '?') + 't=' + new Date().getTime();                   
                        let blob = await fetch(imageUrlWithCacheBuster, {
                            mode: 'cors',
                            headers: {
	...
	return (
		...
		<Toast 
            ...
            error={toastErrorMsg}
        />
		<Header user={user} t={t} from='news' shelter={shelter}/>
    	<main>
    		{/*
